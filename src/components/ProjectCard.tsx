...
const ProjectCard = ({ 
  ...
  return (
    <Transition 
      ...
      delay={100 * index}
      className={cn(
        "project-card group h-[400px] md:h-[450px]",
        className
      )}
    >
        ...
        <div className="flex justify-between items-start">
          <div>
            <h3 className="text-2xl font-bold mb-3">
              {title}
              {isTableau && (
                <span className="ml-2 text-xs py-0.5 px-2 bg-blue-500/20 text-blue-500 rounded-full">
                  Tableau
                </span>
              )}
              {isGithub && (
                <span className="ml-2 text-xs py-0.5 px-2 bg-purple-500/20 text-purple-500 rounded-full">
                  GitHub
                </span>
              )}
            </h3>
            <p className="text-sm text-foreground/80 mb-4 max-w-md">{description}</p>
          ...
          <a 
            ...
            className="p-2 rounded-full bg-primary text-primary-foreground transition-all duration-300 ease-apple transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100"
